...
def LoadMotorFile(MotorFile):
    ...
    try:
        ...
        logger.error(f'Error reading Motor file {MotorFile}: {e}.')
        return None
    ...
    for col in MotColumnsRenames.keys():
        if col not in dfMot.columns:
            logger.error(f'Column {col} not found in {MotorFile}.')
            return None
    ...
    dfMot['Time'] = dfMot['Time'] - dfMot['Time'].iloc[0]
    
    # Corrections Position
    dfMot['Position'] = dfMot['Position'] - dfMot['Position'].min()
...
def LoadDAQFile(DaqFile):
    ...
    try:
        ...
        logger.error(f'Error reading DAQ file {DaqFile}: {e}.')
        return None
    ...
    for col in DaqColumnsRenames.keys():
        if col not in dfDaq.columns:
            if col == 'Current':
                ...
                logger.warning(f'Column Current not found in {DaqFile}. Filled with None.')
            else:
                logger.error(f'Column {col} not found in {DaqFile}.')
                return None
    ...
    try:
        ...
        logger.warning(f'Column Current not provided or not convertible in {DaqFile}.') 
    
    # Corrections Time
    dfDaq['Time'] = dfDaq['Time'] - dfDaq['Time'].iloc[0]
    
    # Corrections Voltage
    j = dfDaq['Bool1'].eq(1).idxmax()
    dfDaq['Voltage'] = dfDaq['Voltage'] - dfDaq['Voltage'][:j].mean()
...
def LoadFiles(MotorFile, DaqFile):
    ...
    logger.info(f'Motor sampling rate: {MotFs}.')
    
    # DAQ sampling rate
    DaqFs = 1 / dfDaq['Time'].diff().mean()
    logger.info(f'DAQ sampling rate: {DaqFs}.')
                    
    # Finding cycles
    MotCycles = FindCycles(dfMot['State'])
    DaqCycles = FindCycles(dfDaq['State'])
    if len(MotCycles) != len(DaqCycles):
        logger.warning(f'Different number of cycles: Motor={len(MotCycles)}, DAQ={len(DaqCycles)}. Using minimum.')
...
#         plt.show()
#     else:
#         logger.info('No folder was selected. Operation canceled.')
