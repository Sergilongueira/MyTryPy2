...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Setup tkinter root for dialogs (hidden)
        self.root = tk.Tk()
        self.root.withdraw()
        self.root.lift()
        ...
        self.tribu_id = simpledialog.askstring(title="Input", prompt="Enter TribuId:", parent=self.root)
    ...
    def toggle_linmot(self):
        ...
        if moveLinMot:
            ...
            self.rload_id = simpledialog.askstring(title="Input", prompt="Enter RloadId:", parent=self.root)
            ...
            self.exp_id = f"{self.date_now[:4]}-{self.tribu_id}-{self.rload_id}"
            
            self.processor.local_path = os.path.join(self.exps_dir, "RawData", self.exp_id)
